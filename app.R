...
server <- function(input, output, session) {
  ...
  observeEvent(input$submit_vote, {
    ...
    # Format both times to the nearest minute (ignore seconds)
    format_time <- function(t) {
      as.POSIXct(format(t, "%Y-%m-%d %H:%M:00"), tz = "Africa/Nairobi")
    }
    
    expiry_time <- format_time(poll$expiry[1])
    now_time <- format_time(Sys.time())
    
    cat("Expiry (DB):", expiry_time, "| Now:", now_time, "\n")
