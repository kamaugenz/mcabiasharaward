...
CREATE TABLE IF NOT EXISTS pigakura (
  ...
  votes TEXT[] DEFAULT '{}',
  expiry TIMESTAMP,  -- ✅ Use TIMESTAMP instead of DATE
...
")),
  ...
  function startCountdown(targetDate) {
    ...
    countdownInterval = setInterval(function() {
      ...
      if (distance < 0) {
        countdownEl.innerText = 'Poll Has Expired';
        countdownEl.style.color = '#B22222'; // Deep red
      ...
      countdownEl.style.color = '';         // Reset color while counting
      countdownEl.style.fontWeight = '';    // Reset weight while counting
    }, 1000);
  }
"))
...
server <- function(input, output, session) {
  ...
  observeEvent(input$submit_vote, {
    ...
    # Fetch poll with expiry timestamp
    poll <- dbGetQuery(db_conn, "
    SELECT uuid, contestants, votes, expiry FROM pigakura WHERE id = $1 LIMIT 1
  ", params = list(fixed_poll_id))
    ...
    # Format expiry and current time
    expiry_str <- format(as.POSIXct(poll$expiry[1]), "%Y-%m-%d %H:%M:%S")
    now_str <- format(Sys.time(), "%Y-%m-%d %H:%M:%S")
    
    expiry_time <- as.POSIXct(expiry_str, format = "%Y-%m-%d %H:%M:%S")
    now_time <- as.POSIXct(now_str, format = "%Y-%m-%d %H:%M:%S")
    
    if (expiry_time < now_time) {
      f7Toast("⚠️ Voting for this poll has ended.", position = "center", closeTimeout = 4000)
      return()
    }
    ...
    # Format vote array for PostgreSQL
    escape_pg_array_element <- function(x) {
