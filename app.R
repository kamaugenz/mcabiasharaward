...
server <- function(input, output, session) {
  ...
  ", params = list(1))
    ...
    print(poll_data$expiry[1])  # ⬅️ Add this for debugging
    
    expiry_date <- as.character(poll_data$expiry[1])
    expiry_datetime <- paste0(expiry_date, "T00:00:00")  # Use if DATE type
  ...
  output$contestants_ui <- renderUI({
    ...
    tagList(
      tags$div(style = "font-weight:bold; font-size:18px; margin-bottom:8px;", poll_data$title[1]),
      ...
      tags$div(style = "margin-bottom:16px;",
               lapply(seq_along(contestants), function(i) {
                 ...
                 tags$div(style = "display:flex; align-items:center; margin-bottom:14px;",
                          tags$input(type = "radio", name = "vote_choice", value = name, id = inputId,
                                     style = "width:20px; height:20px;",
                                     onchange = sprintf("Shiny.setInputValue('vote_choice', '%s')", name)),
                          tags$label(`for` = inputId, style = "margin-left:12px; font-size:18px; font-weight:500;", name))
               })
      ),
  ...
  observeEvent(input$create_poll, {
    ...
    # Escape and quote options for PG array literal
